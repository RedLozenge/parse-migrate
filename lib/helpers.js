...
function createQueryForObjectTemplate(objectTemplate) {
    ...
    for (var key in objectTemplate) {
            ...
            (typeof objectTemplate[key] === 'number'))
        {
            ...
            // See if the template array is full of search commands
            var areSearchCommands = _.map(objectTemplate[key],
                function(object) {
                    return object.hasOwnProperty('__search');
                }
            );

            if (_.contains(areSearchCommands, false)) {
                // If there are no search commands in the array, we are just
                // matching scalar values.
                query.containsAll(key, objectTemplate[key]);
            } else {
                // If it is, then look up each of the templates in the
                // array and add them to the query
                var searchPromises = _.map(objectTemplate[key],
                    function(template) {
                            ...
                            for (key in arguments) {
                                if (Object.keys(arguments[key]).length === 0) {
                                    return Parse.Promise.error(
                                        'could not find matching search dependency');
                                }
                            }
                            var searchObjects = _.flatten(arguments);
        ...
        if (objectProperty.hasOwnProperty('__type')) {
            switch (objectProperty.__type) {
                ...
                default:
                   ...
                   (objectProperty.hasOwnProperty('__searchArray')))
        {
            var singleResult = true;
            if (objectProperty.hasOwnProperty('__searchArray')) {
                singleResult = false;
            }
