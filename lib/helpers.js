 ...
 *     property6: {
 *         // This will find/create a reference to a matching object
 *         __search: 'OtherParseClassName',
 *         someProperty: 'find object matching this property' },
 *     property7: [
 *         {
 *             // This finds/stores an array of matching references
 *             __search: 'OtherParseClassName',
 *             someProperty: 'find object(s) matching this property' }
 *     ]
 *     ...
 * }
 *
...
// Allow us to declare functions inside loops. Pretty essential for Promise
// operations.
/*jshint loopfunc:true */

// Allow us to call functions before they're defined. JS has declaration
// 'hoisting' so this is irrelevant. Allows functions A() and B() to call each
// other.
...
/**
 * Intenral function.
 * Looks through an array of objects to see if they all have the '__search'
 * property set on them. This is used when we are looking to see if an
 * array of templates is an array of references to other objects, because
 * those need to be handled specially.
 *
 * @param {object} templateArray
 * @returns {boolean} Whether or not the array contains __search tempalates.
 */
function isArrayOfSearchTemplates(templateArray) {
    var areSearchTemplates = _.map(templateArray,
        function(object) {
            return object.hasOwnProperty('__search');
        }
    );

    return !(_.contains(areSearchTemplates, false));
}

/**
 * Internal function.
 * Takes an array of search templates and fetches the objects they point to.
 *
 * @param {array} searchTemplates
 * @returns {array} Array of Parse Objects that match the search templates.
 */
function fetchSearchTemplates(searchTemplates) {
    var searchPromises = _.map(searchTemplates,
        function(template) {
            return searchForObjectTemplate(template);
        }
    );
    
    return Parse.Promise.when(searchPromises).then(
        function() {
            for (var key in arguments) {
                if (Object.keys(arguments[key]).length === 0) {
                    var message =
                        'could not find matching search dependency for template: ' +
                        JSON.stringify(searchTemplates[key]);
                    return Parse.Promise.error(new Error(message));
                }
            }
            var searchObjects = _.flatten(arguments, true);
            return Parse.Promise.as(searchObjects);
        }
    );
}
 ...
 * @returns {Parse.Promise} A promise that will contain a Parse.Query object when fulfilled.
 */
function createQueryForObjectTemplate(objectTemplate) {
    ...
    // Some parts of query construction might be asynch, so we will wait for
    // these to return before returning the final query.
    var queryPromises = [];
    ...
    for (var key in objectTemplate) {
            ...
            (typeof objectTemplate[key] === 'number'))
        {
            ...
            // See if the template array is full of search templates
            if (isArrayOfSearchTemplates(objectTemplate[key])) {
                queryPromises.push(
                    fetchSearchTemplates(objectTemplate[key]).then(
                        function(searchObjects) {
                            query.containsAll(key, searchObjects);
                        }
                    )
                );
            } else {
                // If there are no search commands in the array, we are just
            ...
            if (objectTemplate[key].hasOwnProperty('__type')) {
                ...
                queryPromises.push(
                    createQueryForObjectTemplate(objectTemplate[key]).then(
                        function(subQuery) {
    ...
    return Parse.Promise.when(queryPromises).then(
        function() {
...
function searchForObjectTemplate(objectTemplate) {
    return createQueryForObjectTemplate(objectTemplate).then(
        function(query) {
            return query.find();
        },
        function(errors) {
            if (_.isArray(errors)) {
                // Concatenate error messages if there is an array of them
                var messages = _.map(errors,
                    function(possibleError) {
                        if (util.isError(possibleError)) {
                            return possibleError.message + '; ';
                        }
                    }
                );
                return Parse.Promise.error(new Error(messages));
            } else {
                console.log(errors);
                return errors;
            }
...
function createObjectFromTemplate(objectTemplate) {
    ...
    for (var key in objectTemplate) {
        ...
        } else if ((_.isArray(objectProperty)) &&
                   (isArrayOfSearchTemplates(objectProperty)))
        {
            // If this is an array of search templates, let's lookup the
            // references. The strange function syntax is used to hang on to
            // a copy of the variable `key` at the time of iteration. See:
            //   http://jslinterrors.com/dont-make-functions-within-a-loop
            // for a good time.
            (function (keyCopy) {
                promises.push(
                    fetchSearchTemplates(objectProperty).then(
                        function(searchObjects) {
                            object.set(keyCopy, searchObjects);
                        }
                    )
                );
            })(key);
